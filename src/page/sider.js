	...
	render() {
		return (
			<Menu
				...
				defaultOpenKeys={['sub7','sub1', 'sub2', 'sub3', 'sub4', 'sub5', 'sub10']}
				mode="inline"
			>
				<SubMenu key="sub7" title={<span><Icon type="smile-o" /><span>自己的菜单</span></span>}>
					<Menu.Item key="11">
						<Link to={'userMgr'}>用户管理</Link>
					</Menu.Item>
					<Menu.Item key="12">
						<Link to={'label'}>定时任务管理</Link>
					</Menu.Item>
					<Menu.Item key="13">
						<Link to={'label'}>文章管理</Link>
					</Menu.Item>
					<Menu.Item key="14">
						<Link to={'label'}>待开发</Link>
					</Menu.Item>
